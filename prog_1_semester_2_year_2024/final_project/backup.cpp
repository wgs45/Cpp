...
public:
  ...
  */
  ...
  bool isValidMove(int startRow, int startCol, int endRow, int endCol,
    ...
    return (abs(startRow - endRow) <= 1 && abs(startCol - endRow) <= 1);
  }
};
...
public:
  // Start the game loop
  void start() {
    board.printBoard(); // Print the initial state of the chessboard
    string move;        // String to store the user's input move
